...
let currDocumentsId = 1;
let currSectionsId = 1;
let currParagraphsId = 1;

function scrapeFederalistPapers() {
...
function scrape() {
  return Promise.all([
    scrapeFederalistPapers(),
  ]);
}

function write() {
  fs.writeFile('documents.json', JSON.stringify(documentsJson, null, 4), (err)=> console.log('\n\ndocuments successfully written!\n\n'));
  fs.writeFile('sections.json', JSON.stringify(sectionsJson, null, 4), (err)=> console.log('\n\nsections successfully written!\n\n'));
  fs.writeFile('paragraphs.json', JSON.stringify(paragraphsJson, null, 4), (err)=> console.log('\n\nparagraphs successfully written!\n\n'));
}
